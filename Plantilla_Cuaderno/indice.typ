// ======================================================
// FILA INDIVIDUAL DEL ÍNDICE / OUTLINE
// ======================================================
...
#let my-outline(appendix-state, appendix-state-hide-parent, part-state, part-location,part-change,part-counter, main-color, textSize1:none, textSize2:none, textSize3:none, textSize4:none) = {
  show outline.entry: it => {
    ...
    if it.level == 1 {
      ...
      let title-text = {
        if title.has("text") { title.text }
        else { "" }
      }
      
      if (counterInt.first() == 1 and appendix-state != none and not appendix-state-hide-parent ){
        my-outline-row(insetSize: 2pt, textWeight: "bold", textSize: textSize2, textColor:main-color, number: none, title: appendix-state, heading_page: heading_page, location: it.element.location(),sangria: -1cm)
        v(0.5cm, weak: true)
      } 
      ...
      if (title-text == "Bibliografía" or title-text == "Bibliography" or title-text=="Index"){
        ...
        my-outline-row(insetSize: 2pt, textWeight: "bold", textSize: textSize2, textColor:main-color, number: number, title: title, heading_page: heading_page, location: it.element.location(),sangria: 0cm)
        v(0.5cm, weak: true)
      }
    }
    else if it.level ==2 {

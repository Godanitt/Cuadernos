// Archivo generado automáticamente desde ../cuaderno.toml.
// No editar a mano: ejecuta `python -m cuadernos update`.

#let notebook-config = (
  title: "Héroes en la Literatura",
  subtitle: "",
  series: "Series Estudios Literatura",
  volume: "L-01",
  date: datetime.today,
  author: ("Daniel Vázquez Lago",),
  main-color: rgb("#199b72"),
  seccond-color: rgb("#2d8668"),
  third-color: rgb("#558d7b"),
  lang: "es",
  cover: "/cuadernos/Literatura/Heroes/generated/cover-extracted.png",
  format: "fullimage",
  cover-theme: "dark",
  cover-zoom: 1,
  cover-dx: 0cm,
  cover-dy: 0cm,
  cover-text-color: "auto",
  image-index: none,
  list-of-figure-title: "Lista de figuras",
  list-of-table-title: "Lista de tablas",
  supplement-chapter: "Capítulo",
  supplement-part: "Parte",
  font-size: 11pt,
  part-style: 0,
  github-url: "https://github.com/Godanitt/Cuadernos",
  copyright: [],
  lowercase-references: false,
  heading-style-compact: true,
  first-line-indent: false,
)

#let bibliography-file = "/cuadernos/Literatura/Heroes/Bibliografia/referencias.bib"
#let bibliography-enabled = true

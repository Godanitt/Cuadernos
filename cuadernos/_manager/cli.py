...
from .migrate import migrate_part_bibliography, migrate_legacy_layout
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p_build.add_argument("--jobs", type=int, default=None, help="Se conserva por compatibilidad; Tinymist compila en serie para proteger tinymist.lock")
    p_build.add_argument("--rebuild-lock", action="store_true", help="Eliminar y reconstruir tinymist.lock con todos los cuadernos")
    ...
    p_update.add_argument("--jobs", type=int, default=None, help="Se conserva por compatibilidad; Tinymist compila en serie para proteger tinymist.lock")
    p_update.add_argument("--rebuild-lock", action="store_true", help="Eliminar y reconstruir tinymist.lock con todos los cuadernos")
...
def prepare_project(notebooks, *, refresh: bool = True):
    bibliography_changes = migrate_part_bibliography(notebooks)
    if bibliography_changes:
        print(f"Bibliografía por partes: {len(bibliography_changes)} ajustes automáticos.")
    if refresh:
...
def _print_build_results(results) -> bool:
    failed = False
...
def cmd_build(notebooks, args) -> int:
    ...
    failed = _print_build_results(results)
...
def cmd_update(notebooks, args) -> int:
    ...
    print(
        f"Actualización completa: {len(notebooks)} cuadernos detectados; "
        f"{final_errors} errores y {final_warnings} advertencias."
    )
    return 1 if failed or final_errors else 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = project_root()
    if args.command in {"update", "sync", "build", "check", "new"}:
        migration_messages = migrate_legacy_layout(root)
        if migration_messages:
            print(f"Migración automática: {len(migration_messages)} cambios aplicados.")
            for message in migration_messages[:12]:
                print(f"  - {message}")
            if len(migration_messages) > 12:
                print(f"  - … y {len(migration_messages) - 12} cambios más")

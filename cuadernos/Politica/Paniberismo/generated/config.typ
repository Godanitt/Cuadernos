// Archivo generado automáticamente desde ../cuaderno.toml.
// No editar a mano: ejecuta `python -m cuadernos update`.

#let notebook-config = (
  title: "Paniberismo",
  subtitle: "",
  series: "Series Ciencias Políticas",
  volume: "P-01",
  date: datetime.today,
  author: ("Daniel Vázquez Lago",),
  main-color: rgb("#6226c2"),
  seccond-color: rgb("#8152ce"),
  third-color: rgb("#8764c0"),
  lang: "es",
  cover: "/cuadernos/Politica/Paniberismo/generated/cover-extracted.png",
  format: "fullimage",
  cover-theme: "dark",
  cover-zoom: 1,
  cover-dx: 0cm,
  cover-dy: 0cm,
  cover-text-color: black,
  image-index: none,
  list-of-figure-title: "Lista de figuras",
  list-of-table-title: "Lista de tablas",
  supplement-chapter: "Capítulo",
  supplement-part: "Parte",
  font-size: 11pt,
  part-style: 0,
  github-url: "https://github.com/Godanitt/Cuadernos",
  copyright: [],
  lowercase-references: false,
  heading-style-compact: true,
  first-line-indent: false,
)

#let bibliography-file = "/cuadernos/Politica/Paniberismo/Bibliografia/referencias.bib"
#let bibliography-enabled = true

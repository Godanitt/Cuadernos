// Archivo generado automáticamente desde ../cuaderno.toml.
// No editar a mano: ejecuta `python -m cuadernos update`.

#let notebook-config = (
  title: "Geometría Diferencial",
  subtitle: "",
  series: "Series Matemáticas",
  volume: "M-02",
  date: datetime.today,
  author: ("Daniel Vázquez Lago",),
  typography: "Libertinus Serif",
  math-typography: "Libertinus Math",
  main-color: rgb("#dc681a"),
  seccond-color: rgb("#c06d35"),
  third-color: rgb("#d28b5b"),
  lang: "es",
  cover: "/cuadernos/Matematicas/Geometria_Diferencial/generated/cover-extracted.png",
  format: "fullimage",
  cover-theme: "dark",
  cover-zoom: 1,
  cover-dx: 0cm,
  cover-dy: 0cm,
  cover-text-color: black,
  image-index: none,
  list-of-figure-title: "Lista de figuras",
  list-of-table-title: "Lista de tablas",
  supplement-chapter: "Capítulo",
  supplement-part: "Parte",
  font-size: 12pt,
  part-style: 0,
  github-url: "https://github.com/Godanitt/Cuadernos",
  copyright: [],
  lowercase-references: false,
  heading-style-compact: true,
  first-line-indent: false,
)

#let bibliography-file = "/cuadernos/Matematicas/Geometria_Diferencial/Bibliografia/referencias.bib"
#let bibliography-enabled = true
